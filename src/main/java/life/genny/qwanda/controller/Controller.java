	...
	public static void getProject(final Service bes, final String sheetId, final String tables) {
		...
		if (tablesLC.contains("ask")) {
			...
			((HashMap<String, HashMap>) table2Update.get("ask")).entrySet().stream().forEach(map -> {
				try {
					HashMap<String, HashMap> newMap = ((HashMap<String, HashMap>) saveProjectData.get("ask"))
							.get(map.getKey());
		...
		if (tablesLC.contains("datatype")) {
			...
			((HashMap<String, HashMap>) table2Update.get("dataType")).entrySet().stream().forEach(map -> {
				try {
					HashMap<String, HashMap> newMap = ((HashMap<String, HashMap>) saveProjectData.get("dataType"))
							.get(map.getKey());
					if (!newMap.entrySet().containsAll(map.getValue().entrySet())) {
		...
		if (tablesLC.contains("attribute")) {
			...
			if (table2Update.get("dataType") == null)
				table2Update.put("dataType", bl.sheets.newGetDType());
			((HashMap<String, HashMap>) table2Update.get("attributes")).entrySet().stream().forEach(map -> {
				try {
					HashMap<String, HashMap> newMap = ((HashMap<String, HashMap>) saveProjectData.get("attributes"))
							.get(map.getKey());
					if (!newMap.entrySet().containsAll(map.getValue().entrySet())) {
		...
		if (tablesLC.contains("attributelink")) {
			...
			((HashMap<String, HashMap>) table2Update.get("attributelink")).entrySet().stream().forEach(map -> {
				try {
					HashMap<String, HashMap> newMap = ((HashMap<String, HashMap>) saveProjectData.get("attributelink"))
							.get(map.getKey());
					if (!newMap.entrySet().containsAll(map.getValue().entrySet())) {
		...
		if (tablesLC.contains("baseentity")) {
			...
			((HashMap<String, HashMap>) table2Update.get("baseEntitys")).entrySet().stream().forEach(map -> {
				try {
					HashMap<String, HashMap> newMap = ((HashMap<String, HashMap>) saveProjectData.get("baseEntitys"))
							.get(map.getKey());
					if (!newMap.entrySet().containsAll(map.getValue().entrySet())) {
						merge.put(map.getKey(), map);
					}
		...
		if (tablesLC.contains("entityattribute")) {
			...
			((HashMap<String, HashMap>) table2Update.get("attibutesEntity")).entrySet().stream().forEach(map -> {
				try {
					HashMap<String, HashMap> newMap = ((HashMap<String, HashMap>) saveProjectData
							.get("attibutesEntity")).get(map.getKey());
					if (!newMap.entrySet().containsAll(map.getValue().entrySet())) {
		...
		if (tablesLC.contains("entityentity")) {
			...
			((HashMap<String, HashMap>) table2Update.get("basebase")).entrySet().stream().forEach(map -> {
				try {
					HashMap<String, HashMap> newMap = ((HashMap<String, HashMap>) saveProjectData.get("basebase"))
							.get(map.getKey());
					if (!newMap.entrySet().containsAll(map.getValue().entrySet())) {
		...
		if (tablesLC.contains("validation")) {
			...
			((HashMap<String, HashMap>) table2Update.get("validations")).entrySet().stream().forEach(map -> {
				try {
					HashMap<String, HashMap> newMap = ((HashMap<String, HashMap>) saveProjectData.get("validations"))
							.get(map.getKey());
					if (!newMap.entrySet().containsAll(map.getValue().entrySet())) {
		...
		if (tablesLC.contains("question")) {
			...
			((HashMap<String, HashMap>) table2Update.get("questions")).entrySet().stream().forEach(map -> {
				try {
					HashMap<String, HashMap> newMap = ((HashMap<String, HashMap>) saveProjectData.get("questions"))
							.get(map.getKey());
					if (!newMap.entrySet().containsAll(map.getValue().entrySet())) {
		...
		if (tablesLC.contains("questionquestion")) {
			...
			((HashMap<String, HashMap>) table2Update.get("questionQuestions")).entrySet().stream().forEach(map -> {
				try {
					HashMap<String, HashMap> newMap = ((HashMap<String, HashMap>) saveProjectData
							.get("questionQuestions")).get(map.getKey());
					if (!newMap.entrySet().containsAll(map.getValue().entrySet())) {
