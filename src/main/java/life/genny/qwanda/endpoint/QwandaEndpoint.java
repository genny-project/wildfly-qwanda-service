...
public class QwandaEndpoint {
  ...
  public Response addLink(final Link ee) {
    ...
    // ee.setTarget(target);
    // ee.setValue("TEST");
    Log.info("Creating new Link " + ee.getSourceCode() + ":" + ee.getTargetCode() + ":"
        + ee.getAttributeCode() + ":" + ee.getLinkValue());

    EntityEntity newEntityEntity = null;

    try {
      newEntityEntity = service.addLink(ee.getSourceCode(), ee.getTargetCode(),
          ee.getAttributeCode(), ee.getLinkValue(), 1.0);
    } catch (IllegalArgumentException | BadDataException e) {
      // TODO Auto-generated catch block
      e.printStackTrace();
    }
    return Response.created(
        UriBuilder.fromResource(QwandaEndpoint.class).path(String.valueOf(newEntityEntity)).build())
        .build();
  }
  ...
  public Response removeLink(final Link ee) {
    ...
    Log.info("Removing Link " + ee.getSourceCode() + ":" + ee.getTargetCode() + ":"
        + ee.getAttributeCode());
  ...
  public Response moveLink(@PathParam("targetCode") final String targetCode, final Link ee) {
    ...
    Log.info("moving Link " + ee.getSourceCode() + ":" + ee.getTargetCode() + ":"
        + ee.getAttributeCode() + " to new Parent " + targetCode);
    Link newEntityEntity =
