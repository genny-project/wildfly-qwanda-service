...
#--server-config=standalone-full-ha.xml
   ...
   echo "Debug is False";
   /opt/jboss/wildfly/bin/standalone.sh --server-config=standalone-full-ha.xml -Djboss.bind.address.private=$myip  -bmanagement=0.0.0.0 -b 0.0.0.0 -Dadmin.username=${ADMIN_USERNAME} -Dadmin.password=${ADMIN_PASSWORD} -Dpublic.host=${myip}
