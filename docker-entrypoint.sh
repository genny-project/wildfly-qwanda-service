...
if [[ $DEBUG == "TRUE" ]]; then
   ...
   export JAVA_OPTS="${JAVA_OPTS}   -agentlib:jdwp=transport=dt_socket,address=*:8787,server=y,suspend=${DEBUG_SUSPEND:=n}  -Drebel.remoting_plugin=true -Djboss.as.management.blocking.timeout=80000" 
   /opt/jboss/wildfly/bin/standalone.sh --debug  -Djboss.bind.address.private=$myip  -bmanagement=0.0.0.0 -b 0.0.0.0 -Dadmin.username=${ADMIN_USERNAME} -Dadmin.password=${ADMIN_PASSWORD} -Dpublic.host=${myip}  -DHIBERNATE_SHOW_SQL=${HIBERNATE_SHOW_SQL:=false} -DHIBERNATE_HBM2DDL=$HIBERNATE_HBM2DDL -DMYSQL_USER=$MYSQL_USER -DMYSQL_PASSWORD=$MYSQL_PASSWORD -Djava.security.auth.login.config=''  -b 0.0.0.0  -Djboss.as.management.blocking.timeout=80000   --server-config=standalone-full-ha.xml
#--server-config=standalone-full-ha.xml
else
   echo "Debug is False";
   /opt/jboss/wildfly/bin/standalone.sh -Djboss.bind.address.private=$myip  -Djboss.tx.node.id=${hostname} -bmanagement=0.0.0.0 -b 0.0.0.0 -Dadmin.username=${ADMIN_USERNAME} -Dadmin.password=${ADMIN_PASSWORD} -Dpublic.host=${myip}  -Djboss.as.management.blocking.timeout=r810000  --server-config=standalone-full-ha.xml
