...
public class QwandaEndpoint {
  ...
  extends JsonSerializer<JavassistLazyInitializer> {
    ...
    public void serialize(
        final JavassistLazyInitializer initializer,
  ...
  public Response create(final Rule entity) {
    ...
    return Response.created(
        UriBuilder.fromResource(QwandaEndpoint.class)
  ...
  public Response create(final Attribute entity) {
    final Attribute attr = service.upsert(entity);
    return Response.created(
        UriBuilder.fromResource(QwandaEndpoint.class)
        .path(String.valueOf(attr.getId()))
        .build())
        .build();
  }
  ...
  public Response createAsks(
    ...
    final List<Ask> asks = service.createAsksByQuestionCode(questionCode, sourceCode, targetCode);
    final QDataAskMessage askMsgs = new QDataAskMessage(asks.toArray(new Ask[0]));

    final String json = JsonUtils.toJson(askMsgs);
  ...
  @Produces(MediaType.APPLICATION_JSON)
  @Transactional
  public Response createAsks2(
      @PathParam("sourceCode") final String sourceCode,
      @PathParam("questionCode") final String questionCode,
      @PathParam("targetCode") final String targetCode,
      @Context final UriInfo uriInfo) {
    final List<Ask> asks = service.createAsksByQuestionCode2(questionCode, sourceCode, targetCode);
  ...
  public Response createAsksUsingQST(final QDataQSTMessage msg) {
    ...
    final Question rootQuestion = service.findQuestionByCode(msg.getRootQST().getQuestionCode());
  ...
  public Response create(final Answer entity) {
    final QDataAnswerMessage answerMsg = new QDataAnswerMessage(entity);
  ...
  public Response create(final BaseEntity entity) {
    final Long ret = service.insert(entity);
  ...
  public Response findBySearchBE(@Context final UriInfo uriInfo) {
    if (securityService.inRole("admin")
        || securityService.inRole("superadmin")
        || securityService.inRole("dev")
        || GennySettings.DEV_MODE) {
      final BaseEntity searchBE = new BaseEntity("SER_");
      ...
      if (pageStartStr != null) {
        final Attribute attributeInteger = new AttributeInteger("QRY_PAGE_START", "PageStart");
        try {
      ...
      if (pageSizeStr != null) {
        final Attribute attributeInteger = new AttributeInteger("QRY_PAGE_SIZE", "PageSize");
        try {
      ...
      final List<BaseEntity> results = service.findBySearchBE(searchBE);
      ...
      final QDataBaseEntityMessage msg = new QDataBaseEntityMessage(beArr, searchBE.getCode(), null);
      msg.setTotal(total);
  ...
  public Response findBySearchBE3(final String hql) {
    ...
    if (securityService.inRole("admin")
        ...
        || securityService.inRole("dev")
        || GennySettings.DEV_MODE) {
      final List<BaseEntity> results = service.findBySearchBE2(hql);
      BaseEntity[] beArr = new BaseEntity[results.size()];
      beArr = results.toArray(beArr);
      final QDataBaseEntityMessage msg = new QDataBaseEntityMessage(beArr, "GRP_ROOT", null);
      msg.setTotal(0L);
      final String json = JsonUtils.toJson(msg);
      return Response.status(200).entity(json).build();
    } else {
  ...
  public Response findBySearchBE4(final String hql) {
    ...
    if (securityService.inRole("admin")
        ...
        || securityService.inRole("dev")
        || GennySettings.DEV_MODE) {
      final List<Object> results = service.findBySearchBE3(hql);
      final String json = JsonUtils.toJson(results);
      return Response.status(200).entity(json).build();
    } else {
      return Response.status(401).build();
    }
  ...
  public Response findBySearchBE(final BaseEntity searchBE) {
    ...
    if ((!securityService.inRole("admin") && !securityService.inRole("superadmin") && !securityService.inRole("dev"))
        || searchDevMode) { // TODO Remove the true!
      String stakeHolderCode;
      stakeHolderCode =
          "PER_"
              + QwandaUtils.getNormalisedUsername(
                  (String) securityService.getUserMap().get("username"))
              .toUpperCase();

      final Attribute stakeHolderAttribute = new AttributeText("SCH_STAKEHOLDER_CODE", "StakeholderCode");
      final Attribute sourceStakeHolderAttribute =
          new AttributeText("SCH_SOURCE_STAKEHOLDER_CODE", "SourceStakeholderCode");
      try {
        if (searchBE.containsEntityAttribute("SCH_SOURCE_STAKEHOLDER_CODE")) {
          searchBE.addAttribute(sourceStakeHolderAttribute, Double.valueOf(1.0), stakeHolderCode);
        } else {
          searchBE.addAttribute(stakeHolderAttribute, Double.valueOf(1.0), stakeHolderCode);
        }
    ...
    final List<BaseEntity> results = service.findBySearchBE(searchBE);
    System.out.println(
        "search from db takes us to " + (System.nanoTime() - startTime) / 1e6 + "ms");
    Long total = -1L;

    try {
      total = service.findBySearchBECount(searchBE);
      System.out.println(
          "search count takes us to " + (System.nanoTime() - startTime) / 1e6 + "ms");
    } catch (final Exception e) {
      total = -1L;
    }
    ...
    final Optional<EntityAttribute> parentAttribute = searchBE.findEntityAttribute("SCH_SOURCE_CODE");
    if (parentAttribute.isPresent()) {
      parentCode = parentAttribute.get().getValueString();
    }
    final QDataBaseEntityMessage msg = new QDataBaseEntityMessage(beArr, parentCode, null);
    msg.setTotal(total);
    final String json = JsonUtils.toJson(msg);
    return Response.status(200).entity(json).build();
  }
  ...
  public Response fetchBaseEntityByCode(
      @Context final ServletContext servletContext, @PathParam("sourceCode") final String code) {
    ...
    final String json = JsonUtils.toJson(entity);
  ...
  public Response fetchAttribute(@PathParam("code") final String attributeCode) {
    ...
    final Attribute[] atArr = new Attribute[1];
    atArr[0] = attribute;
    final QDataAttributeMessage msg = new QDataAttributeMessage(atArr);
    msg.setToken(securityService.getToken());
    final String json = JsonUtils.toJson(msg);
  ...
  public Response fetchAsksMsgByQuestionCode(@PathParam("questionCode") final String questionCode) {
    ...
    final BaseEntity user = service.getUser();
    final List<Ask> asks = service.createAsksByQuestionCode(questionCode, user.getCode(), user.getCode());
    final QDataAskMessage askMsgs = new QDataAskMessage(asks.toArray(new Ask[0]));
    final String json = JsonUtils.toJson(askMsgs);
    return Response.status(200).entity(json).build();
  }
  ...
  public Response fetchGPSByTargetBaseEntityId(@PathParam("id") final Long id) {
    ...
    final String json = JsonUtils.toJson(items);
    return Response.status(200).entity(json).build();
  }
  ...
  @Produces(MediaType.APPLICATION_JSON)
  @Transactional
  public Response fetchAnswersByTargetBaseEntityCode(
      @PathParam("targetCode") final String targetCode) {
  ...
  public Response getBaseEntitys(@Context final UriInfo uriInfo) {
    ...
    final String json = JsonUtils.toJson(targets);
    return Response.status(200).entity(json).build();
  }
  ...
  public Response getTargets(
    ...
    final List<BaseEntity> targets =
        service.findChildrenByAttributeLink(
            sourceCode, linkCode, includeAttributes, pageStart, pageSize, 1, qparams);
  ...
  @Produces("application/json")
  public Response getTargetsUsingLinkValue(
      @PathParam("sourceCode") final String sourceCode,
      @DefaultValue("LNK_CORE") @PathParam("linkCode") final String linkCode,
      @PathParam("linkValue") final String linkValue,
      @Context final UriInfo uriInfo) {
    // log.info("Entering GET TARGETS
    // /baseentitys/{sourceCode}/linkcodes/{linkCode}/linkValue/{linkValue}");
    Integer pageStart = 0;
    Integer pageSize = 10; // default
    boolean includeAttributes = false;
    ...
    final List<BaseEntity> targets =
        service.findChildrenByLinkValue(
            sourceCode,
            linkCode,
            linkValue,
            includeAttributes,
  ...
  @Produces("application/json")
  public Response getTargetsUsingLinkValueWithAttributes(
      @PathParam("sourceCode") final String sourceCode,
      @DefaultValue("LNK_CORE") @PathParam("linkCode") final String linkCode,
      @PathParam("linkValue") final String linkValue,
      @Context final UriInfo uriInfo) {
  ...
  @Produces("application/json")
  public Response getTargetsWithAttributes(
      @PathParam("sourceCode") final String sourceCode,
      @DefaultValue("LNK_CORE") @PathParam("linkCode") final String linkCode,
      @Context final UriInfo uriInfo) {
    final String stakeholderCode = null;
    ...
    final List<BaseEntity> targets =
        service.findChildrenByAttributeLink(
            sourceCode, linkCode, true, pageStart, pageSize, level, qparams, stakeholderCode);
    ...
    final Long total =
        service.findChildrenByAttributeLinkCount(sourceCode, linkCode, qparams, stakeholderCode);
  ...
  @Produces("application/json")
  public Response getTargetsWithAttributesAndStakeholderCode(
      @PathParam("sourceCode") final String sourceCode,
      @DefaultValue("LNK_CORE") @PathParam("linkCode") final String linkCode,
    ...
    final SearchEntity searchBE =
        new SearchEntity("Targets", "Targets")
    ...
    final Long total =
        service.findChildrenByAttributeLinkCount(
            sourceCode, linkCode, qparams); // TODO add stakeholder
    // filtering
  ...
  public Response importKeycloakUsers(
      @QueryParam("keycloakurl") final String keycloakUrl,
    ...
    final Long usersAddedCount = 0L; // service.importKeycloakUsers(keycloakUrl,
    // realm, username,
    // password, clientId,
  ...
  public Response updateBaseEntity(final BaseEntity baseEntity) {
    ...
    final BaseEntity be = service.findBaseEntityByCode(baseEntity.getCode());
    be.setName(baseEntity.getName());
    be.setBaseEntityAttributes(baseEntity.getBaseEntityAttributes());
    final Long result = service.update(be);
  ...
  public Response forceBaseEntityAttributes(final BaseEntity baseEntity) {
    securityService.getUserMap();
    final String username = (String) securityService.getUserMap().get("username");
    final String userCode = "PER_" + QwandaUtils.getNormalisedUsername(username);
    ...
    if (securityService.inRole("admin")
      ...
      for (final EntityAttribute ea : be.getBaseEntityAttributes()) { // go through wanted survivors
        final Optional<EntityAttribute> optEA = baseEntity.findEntityAttribute(ea.getAttributeCode());
        if (!optEA.isPresent()) {
          service.removeEntityAttribute(ea);
          log.info(
              "Removed attribute " + ea.getAttributeCode() + " for be " + baseEntity.getCode());
        }
      }
      // get updated one
      be = service.findBaseEntityByCode(baseEntity.getCode());
      service.writeToDDT(be);
      final QEventAttributeValueChangeMessage msg =
          new QEventAttributeValueChangeMessage(
              be.getCode(), be.getCode(), be, securityService.getToken());
  ...
  @Produces("application/json")
  public Response removeEntityAttribute(
      @PathParam("baseEntityCode") final String baseEntityCode,
      @PathParam("attributeCode") final String attributeCode) {
    final String username = (String) securityService.getUserMap().get("preferred_username");
    final String userCode = QwandaUtils.getNormalisedUsername(username);

    if ((!securityService.inRole("admin") && !securityService.inRole("superadmin") && !securityService.inRole("dev"))
        || userCode.equalsIgnoreCase(baseEntityCode)) { // TODO Remove the true!
  ...
  public Response removeEntityAttributes(@PathParam("attributeCode") final String attributeCode) {
    final String username = (String) securityService.getUserMap().get("preferred_username");
    QwandaUtils.getNormalisedUsername(username);

    if (!securityService.inRole("admin") && !securityService.inRole("superadmin") && !securityService.inRole("dev")) { // TODO Remove the true!
  ...
  public Response addLink(final Link ee) {
    ...
    return Response.created(
        UriBuilder.fromResource(QwandaEndpoint.class)
        .path(String.valueOf(newEntityEntity))
        .build())
        .build();
  }
  ...
  public Response moveLink(@PathParam("targetCode") final String targetCode, final Link ee) {
    Log.info(
        "moving Link "
            ...
            + " to new Parent "
            + targetCode);
    final Link newEntityEntity =
        service.moveLink(ee.getSourceCode(), ee.getTargetCode(), ee.getAttributeCode(), targetCode);
  ...
  @Produces(MediaType.APPLICATION_JSON)
  @Transactional
  public Response fetchLinks(
      @PathParam("targetCode") final String targetCode,
      @PathParam("linkCode") final String linkCode) {
    final List<Link> items = service.findLinks(targetCode, linkCode);
    final Link[] array = items.toArray(new Link[0]);
    final String json = JsonUtils.toJson(array);
    return Response.status(200).entity(json).build();
  }
  ...
  @Produces(MediaType.APPLICATION_JSON)
  @Transactional
  public Response fetchLinksCount(
      @PathParam("targetCode") final String targetCode,
      @PathParam("linkCode") final String linkCode) {
  ...
  @ApiOperation(
      value = "baseentitys/{targetCode}/links/children/count",
      notes = "Links children count")
  @Produces(MediaType.APPLICATION_JSON)
  @Transactional
  public Response fetchChildLinksCount(
      @PathParam("targetCode") final String targetCode,
      @PathParam("linkCode") final String linkCode) {
  ...
  @Produces(MediaType.APPLICATION_JSON)
  @Transactional
  public Response findParentLinksCount(
      @PathParam("targetCode") final String targetCode,
      @PathParam("linkCode") final String linkCode) {
  ...
  @Produces(MediaType.APPLICATION_JSON)
  @Transactional
  public Response fetchChildLinks(
      @PathParam("sourceCode") final String sourceCode,
      @PathParam("linkCode") final String linkCode) {
  ...
  @ApiOperation(
      value = "baseentitys/{sourceCode}/linkcodes/{linkCode}/children/{linkValue}",
      notes = "Links")
  @Produces(MediaType.APPLICATION_JSON)
  @Transactional
  public Response fetchChildLinks(
  ...
  @Produces(MediaType.APPLICATION_JSON)
  @Transactional
  public Response fetchParentLinks(
      @PathParam("targetCode") final String targetCode,
      @PathParam("linkCode") final String linkCode) {
  ...
  @ApiOperation(
      value = "baseentitys/{targetCode}/linkcodes/{linkCode}/parents/{linkValue}",
      notes = "Links")
  @Produces(MediaType.APPLICATION_JSON)
  @Transactional
  public Response fetchParentLinks(
  ...
  public Response syncrealms() {
    ...
    final String keycloakRealms = SecureResources.reload();
  ...
  public Response fetchrealms() {
    ...
    final String keycloakRealms = SecureResources.fetchRealms();
    return Response.status(200).entity(keycloakRealms).build();
  }
  ...
  public Response synchronize(@PathParam("table") final String table) {
    ...
    try {
      response =
          QwandaUtils.apiGet(
              GennySettings.QWANDA_SERVICE_URL + "/qwanda/synchronizesheet/" + table,
  ...
  public Response delete(@PathParam("table") final String table) {
    ...
    try {
      response =
          QwandaUtils.apiGet(
              GennySettings.QWANDA_SERVICE_URL + "/qwanda/deletesheet/" + table,
              securityService.getToken(),
              240);
    } catch (final Exception e) {
      e.printStackTrace();
  ...
  public Response getMessageTemplates(@PathParam("templateCode") final String code) {
    final QBaseMSGMessageTemplate template = service.findTemplateByCode(code);

    final String json = JsonUtils.toJson(template);
    return Response.status(200).entity(json).build();
  }
}
