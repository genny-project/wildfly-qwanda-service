...
public class ServiceEndpoint {
  ...
  public Response importKeycloakUsers(@QueryParam("keycloakurl") final String keycloakUrl,
    ...
    final Long usersAddedCount = 0L;

    if (securityService.inRole("superadmin") || securityService.inRole("dev") || GennySettings.DEV_MODE) {
  ...
  public Response synchronizeCacheBEs() {
    ...
    if (securityService.inRole("superadmin") || securityService.inRole("dev") || GennySettings.DEV_MODE) {
      ...
      results = em.createQuery("SELECT distinct be FROM BaseEntity be JOIN  be.baseEntityAttributes ea ")
          .getResultList();
      for (final BaseEntity be : results) {
  ...
  public Response fetchCurrentRouteStatusByPercentageDistance(@PathParam("origin") final String originAddress,
    ...
    if (securityService.inRole("superadmin") || securityService.inRole("dev") || GennySettings.DEV_MODE) {
      ...
      if (googleApiKey == null) {
        ...
        final BaseEntity project = service.findBaseEntityByCode(projectCode);
        final Optional<EntityAttribute> ea = project.findEntityAttribute("PRI_GOOGLE_API_KEY");
        if (ea.isPresent()) {
      ...
      if (googleApiKey != null) {
        final GPSLocation origin = GPSUtils.getGPSLocation(originAddress, googleApiKey);
        final GPSLocation end = GPSUtils.getGPSLocation(destinationAddress, googleApiKey);
        final GPSRoute route = GPSUtils.getRoute(origin, end, googleApiKey);

        final GPSRouteStatus status = GPSUtils.fetchCurrentRouteStatusByPercentageDistance(route, percentage);
  ...
  public Response fetchCurrentRouteStatusByPercentageDuration(@PathParam("origin") final String originAddress,
    ...
    if (securityService.inRole("superadmin") || securityService.inRole("dev") || GennySettings.DEV_MODE) {
      ...
      if (googleApiKey != null) {
        ...
        final GPSRouteStatus status = GPSUtils.fetchCurrentRouteStatusByDuration(route, currentSeconds);
  ...
  public Response executeRuleGroup(@PathParam("rulegroup") final String rulegroup) {
    final String returnMessage = "rule group  fired";
    ...
    if (securityService.inRole("superadmin") || securityService.inRole("dev") || GennySettings.DEV_MODE) {
      ...
      final QEventSystemMessage event = new QEventSystemMessage("FOCUS_RULE_GROUP", properties, token);
  ...
  public Response create(final BaseEntity entity) {
    ...
    if (securityService.inRole("superadmin") || securityService.inRole("dev") || GennySettings.DEV_MODE) {
      BaseEntity be3 = null;
      final String userCode = securityService.getUserCode();
      try {
      ...
      final BaseEntity entity2 = service.addAttributes(entity);
      if (be3==null) {
      ...
      for (final EntityAttribute ea : entity.getBaseEntityAttributes()) {
        final Answer answer = new Answer(userCode,entity.getCode(),ea.getAttributeCode(),ea.getAsString());
      ...
      try {
        ...
        if (be.getCode().startsWith("PER_")) {
          final Optional<EntityAttribute> optUsername = be.findEntityAttribute("PRI_USERNAME");
          if (optUsername.isPresent()) {
            final String newUsername = optUsername.get().getAsString();

            final Optional<EntityAttribute> optFirstname = be.findEntityAttribute("PRI_FIRSTNAME");
            if (optFirstname.isPresent()) {
              final String newFirstname = optFirstname.get().getAsString();

              final Optional<EntityAttribute> optLastname = be.findEntityAttribute("PRI_LASTNAME");
              if (optLastname.isPresent()) {
                final String newLastname = optLastname.get().getAsString();

                final Optional<EntityAttribute> optEmail = be.findEntityAttribute("PRI_EMAIL");
                if (optEmail.isPresent()) {
                  final String newEmail = optEmail.get().getAsString();

                  final String serviceToken = service.getServiceToken(securityService.getRealm());

                  service.getKeycloakUrl(securityService.getRealm());
